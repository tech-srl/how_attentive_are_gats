...
def sparse_rgat2_layer(node_embeddings: tf.Tensor,
    ...
    * E: number of edges of a given edge type
    ...
    edge_type_to_state_transformation_layers_tgt = []
    edge_type_to_state_transformation_layers_value = []
        ...
        if not share_weights:
            edge_type_to_state_transformation_layers_tgt.append(
                tf.keras.layers.Dense(units=state_dim,
                                      use_bias=False,
                                      activation=None,
                                      name="Edge_%i_Weight_tgt_" % edge_type_idx))
            edge_type_to_state_transformation_layers_value.append(
                tf.keras.layers.Dense(units=state_dim,
                                      use_bias=False,
                                      activation=None,
                                      name="Edge_%i_Weight_value_" % edge_type_idx))
        edge_type_to_attention_parameters.append(
            tf.get_variable(shape=(state_dim),
    ...
    for _ in range(num_timesteps):
        ...
        for edge_type_idx, adjacency_list_for_edge_type in enumerate(adjacency_lists):
            ...
            if share_weights:
                transformed_states_tgt = transformed_states
                transformed_states_values = transformed_states
            else:
                transformed_states_tgt = \
                    edge_type_to_state_transformation_layers_tgt[edge_type_idx](cur_node_states)  # Shape [V, D]
                transformed_states_values = \
                    edge_type_to_state_transformation_layers_value[edge_type_idx](cur_node_states)  # Shape [V, D]
            ...
            edge_transformed_source_values = \
                tf.nn.embedding_lookup(params=transformed_states_values, ids=edge_sources)  # Shape [E, D]
            ...
            per_edge_per_head_transformed_states = \
                per_edge_per_head_transformed_source_states \
                + tf.reshape(edge_transformed_target_states, shape=(-1, num_heads, per_head_dim))  # Shape [E, K, D/K]
            # per_edge_per_head_transformed_states = \
            ...
            per_head_attention_pars = tf.reshape(edge_type_to_attention_parameters[edge_type_idx],
                                                 shape=(num_heads, per_head_dim))  # Shape [K, 2*D/K]
            # per_head_attention_pars = tf.reshape(edge_type_to_attention_parameters[edge_type_idx],
            #                                      shape=(num_heads, 2 * per_head_dim))  # Shape [K, 2*D/K]
            # LeakyReLU before dot product
            per_edge_per_head_transformed_states = tf.nn.leaky_relu(per_edge_per_head_transformed_states)
            per_edge_per_head_attention_coefficients = \
                tf.einsum('vki,ki->vk',
                                           per_edge_per_head_transformed_states,
                                           per_head_attention_pars)  # Shape [E, K]

            edge_type_to_per_head_messages.append(tf.reshape(edge_transformed_source_values, shape=(-1, num_heads, per_head_dim)))
