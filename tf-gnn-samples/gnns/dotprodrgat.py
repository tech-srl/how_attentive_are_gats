...
def sparse_dotprodrgat_layer(node_embeddings: tf.Tensor,
    ...
    * E: number of edges of a given edge type
    ...
    edge_type_to_state_transformation_layers_src = []  # Layers to compute the message from a source state
    edge_type_to_state_transformation_layers_tgt = []  # Layers to compute the message from a source state
    edge_type_to_src_value_layer = []  # Layers to compute the message from a source state
    ...
    for edge_type_idx, adjacency_list_for_edge_type in enumerate(adjacency_lists):
        if share_weights:
            qk_layer = tf.keras.layers.Dense(units=state_dim,
                                      use_bias=False,
                                      activation=None,
                                      name="Edge_%i_Weight_src_tgt" % edge_type_idx)
            edge_type_to_state_transformation_layers_src.append(qk_layer)
            edge_type_to_state_transformation_layers_tgt.append(qk_layer)
        else:
            edge_type_to_state_transformation_layers_src.append(tf.keras.layers.Dense(units=state_dim,
                                      use_bias=False,
                                      activation=None,
                                      name="Edge_%i_Weight_src" % edge_type_idx))
            edge_type_to_state_transformation_layers_tgt.append(
                tf.keras.layers.Dense(units=state_dim,
                                      use_bias=False,
                                      activation=None,
                                      name="Edge_%i_Weight_tgt" % edge_type_idx))
        edge_type_to_src_value_layer.append(
            tf.keras.layers.Dense(units=state_dim,
                                  use_bias=False,
                                  activation=None,
                                  name="Edge_%i_Weight_src_value" % edge_type_idx))
    ...
    for _ in range(num_timesteps):
        ...
        for edge_type_idx, adjacency_list_for_edge_type in enumerate(adjacency_lists):
            ...
            transformed_states_src = \
                edge_type_to_state_transformation_layers_src[edge_type_idx](cur_node_states)  # Shape [V, D]
            transformed_states_tgt = \
                edge_type_to_state_transformation_layers_tgt[edge_type_idx](cur_node_states)  # Shape [V, D]
            value_state_src = \
                edge_type_to_src_value_layer[edge_type_idx](cur_node_states)  # Shape [V, D]

            edge_transformed_source_states = \
                tf.nn.embedding_lookup(params=transformed_states_src, ids=edge_sources)  # Shape [E, D]
            edge_transformed_target_states = \
                tf.nn.embedding_lookup(params=transformed_states_tgt, ids=edge_targets)  # Shape [E, D]
            edge_transformed_value_states = \
                tf.nn.embedding_lookup(params=value_state_src, ids=edge_sources)  # Shape [E, D]

            per_edge_per_head_transformed_source_states = \
                tf.reshape(edge_transformed_source_states, shape=(-1, num_heads, per_head_dim))
            per_edge_per_head_transformed_target_states = \
                tf.reshape(edge_transformed_target_states, shape=(-1, num_heads, per_head_dim))
            per_edge_per_head_transformed_src_values = \
                tf.reshape(edge_transformed_value_states, shape=(-1, num_heads, per_head_dim))


            dot_prod_key_val = tf.reduce_sum(
                per_edge_per_head_transformed_source_states * per_edge_per_head_transformed_target_states, axis=-1) / \
                               tf.sqrt(tf.cast(per_head_dim, dtype=tf.float32))
            ...
            #     tf.nn.leaky_relu(dot_prod_key_val)  # Shape [E, K]
            per_edge_per_head_attention_coefficients = dot_prod_key_val

            edge_type_to_per_head_messages.append(per_edge_per_head_transformed_src_values)
