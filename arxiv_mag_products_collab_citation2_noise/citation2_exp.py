...
def train(model, predictor, x, pos_loader, neg_loader, optimizer, device):
    ...
    pbar = tqdm(total=1000, leave=False, dynamic_ncols=True)
...
def test(model, predictor, x, subgraph_loader, split_edge, evaluator,
    ...
    h = model.inference(x, subgraph_loader).to(device)
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='ogbl-citation2', choices=['ogbl-citation2'])
    ...
    model = args.type.get_model(in_channels=data.x.size(-1), 
                                ...
                                use_resdiual_linear=args.use_resdiual_linear
                                ).to(device)
